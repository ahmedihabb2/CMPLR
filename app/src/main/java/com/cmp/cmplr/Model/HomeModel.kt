...
class HomeModel {
    ...
    suspend fun listReturn(): ListBooleanPair {
        ...
        var jsonlist = thebody!!.getAsJsonObject("response")
        ...
        for (item in jsonlist) {
            ...
            //Log.d("back_content",)
            val blog_data: Blog = Blog(
                single_blog["avatar"].toString().replace("\"", ""),
                single_blog["avatar_shape"].toString(),
                single_blog["blog_id"].toString().toInt(),
                single_blog["blog_name"].toString().replace("\"", ""),
                single_blog["follower"].toString().toBoolean(),
